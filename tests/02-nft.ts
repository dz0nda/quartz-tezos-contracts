...
describe('[NFT] Contracts deployment', async () => {
  ...
  it('NFT contract deployment should succeed', async () => {
    await nft.deploy(alice.get_address(), permits.get_address(), aleph_token.get_address(), sync.get_address(), { as: alice })
  });
});

describe('[NFT] Contract configuration', async () => {
  it("Add FA2 as permit consumer", async () => {
    await permits.manage_consumer(new add(nft.get_address()), { as: alice })
  })
...
describe('[NFT] Minting', async () => {
  it('Mint tokens on FA2 as owner for owner should succeed', async () => {
    await aleph_token.update_operators([Or.Left(new operator_param(alice.get_address(), nft.get_address(), aleph_token_id))], { as: alice })
    await nft.mint(
    ...
    await aleph_token.update_operators([Or.Right(new operator_param(alice.get_address(), nft.get_address(), aleph_token_id))], { as: alice })
    const owner = await nft.get_ledger_value(token_id)
  ...
  it('Mint tokens on FA2 as owner for someone else should succeed', async () => {
    ...
    await aleph_token.update_operators([Or.Left(new operator_param(carl.get_address(), nft.get_address(), aleph_token_id))], { as: carl })
    await nft.mint(
    ...
    await aleph_token.update_operators([Or.Right(new operator_param(carl.get_address(), nft.get_address(), aleph_token_id))], { as: carl })
    const owner = await nft.get_ledger_value(token_id.plus(new Nat(1)))
...
describe('[NFT] Transfers', async () => {
  it('Transfer a token not owned should fail', async () => {
    await expect_to_fail(async () => {
      await nft.transfer([new transfer_param(
        alice.get_address(),
        [new transfer_destination(bob.get_address(), new Nat(777), amount)])],
        { as: alice });
    }, nft.errors.FA2_TOKEN_UNDEFINED);
  });
  ...
  it('Transfer more tokens than owned should fail', async () => {
    await expect_to_fail(async () => {
      await nft.transfer([new transfer_param(
        ...
        { as: alice });
    }, nft.errors.FA2_INSUFFICIENT_BALANCE);
  });
  ...
  it('Transfer tokens with an operator and with permit (permit not consumed) should succeed', async () => {
    ...
    const token_owner = await nft.get_ledger_value(token_id)
    assert(token_owner?.equals(alice.get_address()))

    await nft.transfer(tps, { as: carl })

    const token_owner_after = await nft.get_ledger_value(token_id)
  ...
  it('Transfer tokens without an operator and a valid permit (permit consumed)', async () => {
    ...
    const token_owner = await nft.get_ledger_value(another_token)
    assert(token_owner?.equals(alice.get_address()), "Invalid owner before")

    await nft.transfer(tps, { as: carl })

    const token_owner_after = await nft.get_ledger_value(another_token)
...
describe('[NFT] Transfers gasless ', async () => {
  ...
  it('Transfer tokens with from different signer should fail', async () => {
    ...
    const token_owner = await nft.get_ledger_value(new_token)
  ...
  it('Transfer tokens with permit should succeed', async () => {
    ...
    const token_owner_after = await nft.get_ledger_value(new_token)
    assert(token_owner_after?.equals(bob.get_address()), "Invalid owner after")
  });
});
...
describe('[NFT] Transfers one-step ', async () => {
  it('Transfer a token not owned should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await nft.permit_transfer(another_tps, lpermit, { as: bob })
    }, nft.errors.SIGNER_NOT_FROM)
  ...
  it('Transfer a token from another user with wrong a permit should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await nft.permit_transfer(another_tps, lpermit, { as: alice })
    }, get_missigned_error(another_permit_data))
  });
  ...
  it('Transfer more tokens than owned should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await nft.permit_transfer(tps, lpermit, { as: alice })
    }, nft.errors.FA2_INSUFFICIENT_BALANCE)
  });
...
describe('[NFT] Set metadata', async () => {
  it('Set metadata with empty content should succeed', async () => {
    const metadata_before = await nft.get_metadata_value("key")
    assert(metadata_before == undefined);

    await nft.set_metadata("key", Option.Some(new Bytes("")), { as: alice })

    const metadata_after = await nft.get_metadata_value("key")
    assert(metadata_after?.equals(new Bytes("")));
  });

  it('Set metadata called by not owner should fail', async () => {
    await expect_to_fail(async () => {
      await nft.set_metadata("key", Option.Some(new Bytes("")), { as: bob })
    }, nft.errors.INVALID_CALLER);
  });
  ...
  it('Set metadata with valid content should succeed', async () => {
    ...
    await nft.set_metadata("key", Option.Some(data), { as: alice })
...
describe('[NFT] Burn', async () => {
  it('Burn without tokens should fail', async () => {
    await expect_to_fail(async () => {
      await nft.burn(token_id, { as: alice })
    }, nft.errors.FA2_NOT_OWNER);
  });
  ...
  it('Burn tokens with enough tokens should succeed', async () => {
    ...
    const owner = await nft.get_ledger_value(new_token)
    assert(owner?.equals(alice.get_address()))

    await nft.burn(new_token, { as: alice })

    const new_owner = await nft.get_ledger_value(new_token)
...
describe('[NFT] Pause', async () => {
  it('Set FA2 on pause should succeed', async () => {
    await nft.pause({ as: alice });
    const is_paused = await nft.get_paused()
    assert(is_paused);
  });
  ...
  it('Minting is not possible when contract is paused should fail', async () => {
    ...
    }, nft.errors.CONTRACT_PAUSED);
  });

  it('Update operators is not possible when contract is paused should fail', async () => {
    await expect_to_fail(async () => {
      await nft.update_operators([
        Or.Left(new operator_param(alice.get_address(), nft.get_address(), token_id))
      ], { as: alice })
    }, nft.errors.CONTRACT_PAUSED);
  });
  ...
  it('Burn is not possible when contract is paused should fail', async () => {
    await expect_to_fail(async () => {
      await nft.burn(new Nat(1), { as: alice })
    }, nft.errors.CONTRACT_PAUSED);
  });

  it('Unpause by not owner should fail', async () => {
    await expect_to_fail(async () => {
      await nft.unpause({ as: bob });
    }, nft.errors.INVALID_CALLER);
  });

  it('Unpause by owner should succeed', async () => {
    await nft.unpause({ as: alice });
...
describe('[NFT] Transfer ownership', async () => {
  ...
  it('Transfer ownership when contract is paused should succeed', async () => {
    const owner = await nft.get_owner()
    assert(owner.equals(alice.get_address()));
    await nft.declare_ownership(alice.get_address(), { as: alice });
    const new_owner = await nft.get_owner()
    assert(owner.equals(new_owner));
  });

  it('Transfer ownership as non owner should fail', async () => {
    await expect_to_fail(async () => {
      await nft.declare_ownership(bob.get_address(), { as: bob });
    }, nft.errors.INVALID_CALLER);
  });
  ...
  it('Transfer ownership as owner should succeed', async () => {
    const owner = await nft.get_owner()
    assert(owner.equals(alice.get_address()));
    await nft.declare_ownership(bob.get_address(), { as: alice })
    await nft.claim_ownership({ as: bob });
    const new_owner = await nft.get_owner()
...
describe('[NFT] Balance of', async () => {
  ...
  it('Unknown token_id should fail', async () => {
    ...
    await expect_to_fail(async () => {
      await nft.balance_of([new balance_of_request(alice.get_address(), fake_token)], { as: alice })
    }, nft.errors.FA2_TOKEN_UNDEFINED)
  });
